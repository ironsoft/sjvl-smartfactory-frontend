...
export type VlAssemblyInspectionEntryTarget =
  | "vl_assembly_process"
  | "vl_assembly_module"
  | "vl_assembly_sj_no";

/**
 * VL Assembly Process / Module / SJ No 상세에서 검사 기록 작성 화면으로 가는 QR (URL prefill).
 */
export default function VlAssemblyInspectionEntryQr({
  targetParam,
  pk,
  disabled,
  ...
  targetParam: VlAssemblyInspectionEntryTarget;
  pk: number;
  disabled?: boolean;
  ...
  /** 검사 QR은 일별 실적 QR(흰 배경·실선 파랑)과 구분: 은은한 패드 + 점선 테두리 */
  const inspectionQrPadBg = useColorModeValue("gray.50", "gray.700");
  const inspectionQrFrameBorder = useColorModeValue("gray.400", "gray.500");
  const [qrDataUrl, setQrDataUrl] = useState("");

  const inspectionUrl = useMemo(
    () =>
      typeof window !== "undefined" && Number.isFinite(pk) && pk >= 1
        ? `${window.location.origin}/vl-assembly-production/inspections/new?${targetParam}=${pk}`
        : "",
    [targetParam, pk]
  );
  ...
  const body = (
    <>
      ...
      >
        <Box
          flexShrink={0}
          bg={inspectionQrPadBg}
          p={2}
          borderRadius="lg"
          border="2px dashed"
          borderColor={inspectionQrFrameBorder}
        >
          <Box
            as="img"
            src={qrDataUrl}
