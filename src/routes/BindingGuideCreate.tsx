...
export default function BindingGuideCreate() {
  ...
  const generateSerialNumber = () => {
    ...
    setValue("serial_number", `BG-${random}`, { shouldValidate: true });
  };
  ...
  const uploadOneVideo = async (file: File, bindingGuideId: string) => {
    ...
    await createBindingGuideVideo({ VideoFile: videoURL, ThumbnailFile: thumbnailURL, bindingGuideId });
  };

  const uploadOnePhoto = async (file: File, bindingGuideId: string, description: string) => {
    ...
    await createBindingGuidePhoto({ file: cfUrl, bindingGuideId, description });
  };

  const mutation = useMutation({
    mutationFn: createBindingGuide,
    onSuccess: async (data: { id?: string; name?: string }) => {
      queryClient.invalidateQueries({ queryKey: ["bindingGuides"] });
      const bindingGuideId = data?.id;

      if (bindingGuideId && selectedFiles.length > 0) {
        setIsUploadingPhotos(true);
        try {
          for (const file of selectedFiles) {
            await uploadOnePhoto(file, bindingGuideId, data.name ?? "");
          }
          queryClient.invalidateQueries({ queryKey: ["bindingGuidePhotos", bindingGuideId] });
        } catch {
          toast({
            title: t("bindingGuideCreate.photosFailed"),
            status: "warning",
            duration: 3000,
            isClosable: true,
      ...
      if (bindingGuideId && selectedVideoFiles.length > 0) {
        setIsUploadingVideos(true);
        try {
          for (const file of selectedVideoFiles) {
            await uploadOneVideo(file, bindingGuideId);
          }
          queryClient.invalidateQueries({ queryKey: ["bindingGuideVideos", bindingGuideId] });
        } catch {
          toast({
            title: t("bindingGuideCreate.videosFailed"),
            status: "warning",
            duration: 3000,
            isClosable: true,
            position: "bottom-right"
      ...
      toast({
        title: t("bindingGuideCreate.created"),
        status: "success",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
      ...
      if (bindingGuideId) navigate(`/binding-guides/${bindingGuideId}`);
      else navigate("/binding-guides");
    },
    onError: (error: any) => {
      ...
      if (data?.serial_number) {
        ...
        toast({
          title: t("bindingGuideCreate.createFailed"),
          description: msg,
          status: "error",
          duration: 4000,
          isClosable: true,
      ...
      const firstMsg = data ? Object.values(data).flat().join(" / ") : t("bindingGuideCreate.error");
      toast({
        title: t("bindingGuideCreate.createFailed"),
  ...
  return (
    ...
    <Box maxW={{ base: "3xl", lg: "2xl" }} mx="auto" bg={cardBg} borderRadius="lg" p={{ base: 4, md: 6 }} boxShadow="sm">
      <Button as={RouterLink} to="/binding-guides" leftIcon={<FaArrowLeft />} variant="ghost" size="sm" mb={4}>
        {t("bindingGuideCreate.backToList")}
      </Button>

      <Heading size="md" mb={6}>{t("bindingGuideCreate.title")}</Heading>
      ...
      <VStack as="form" spacing={4} align="stretch" onSubmit={handleSubmit(onSubmit)}>
        ...
        <FormControl isInvalid={!!errors.name} isRequired>
          <FormLabel>{t("bindingGuideCreate.name")}</FormLabel>
          <Input {...register("name", { required: t("bindingGuideCreate.nameRequired") })} placeholder={t("bindingGuideCreate.namePlaceholder")} size="sm" />
        </FormControl>

        <FormControl isInvalid={!!errors.serial_number} isRequired>
          <FormLabel>{t("bindingGuideCreate.serialNumber")}</FormLabel>
          <HStack spacing={2}>
            <InputGroup size="sm" flex={1}>
              <Input
                {...register("serial_number", { required: t("bindingGuideCreate.serialRequired") })}
                placeholder={t("bindingGuideCreate.serialPlaceholder")}
              />
            </InputGroup>
            <Button
              size="sm"
              ...
              flexShrink={0}
            >
              {t("bindingGuideCreate.autoGenerate")}
            </Button>
          </HStack>
          {errors.serial_number ? (
            <FormErrorMessage>{errors.serial_number.message}</FormErrorMessage>
          ) : (
            <FormHelperText>{t("bindingGuideCreate.serialHelper")}</FormHelperText>
          )}
        </FormControl>

        <FormControl isRequired>
          <FormLabel>{t("bindingGuideCreate.status")}</FormLabel>
          <Select {...register("status")} size="sm">
            <option value="">{t("bindingGuideCreate.selectStatus")}</option>
            <option value="in_use">{t("bindingGuideCreate.statusInUse")}</option>
            <option value="obsolete">{t("bindingGuideCreate.statusObsolete")}</option>
            <option value="removed">{t("bindingGuideCreate.statusRemoved")}</option>
            <option value="lost">{t("bindingGuideCreate.statusLost")}</option>
          </Select>
        </FormControl>

        <FormControl>
          <FormLabel>{t("bindingGuideCreate.description")}</FormLabel>
          <Textarea {...register("description")} placeholder={t("bindingGuideCreate.descriptionPlaceholder")} size="sm" rows={3} />
        </FormControl>

        <FormControl>
          <FormLabel>{t("bindingGuideCreate.location")}</FormLabel>
          <Select {...register("location")} size="sm">
            <option value="">{t("bindingGuideCreate.location")}</option>
            {locations.map((loc) => (
        ...
        <FormControl>
          <FormLabel>{t("bindingGuideCreate.photos")}</FormLabel>
          <Input
            ref={fileInputRef}
            type="file"
            accept="image/*"
          ...
          />
          <FormHelperText>{t("bindingGuideCreate.photosHelper")}</FormHelperText>
        </FormControl>
        ...
        <FormControl>
          <FormLabel>{t("bindingGuideCreate.videos")}</FormLabel>
          <Input
            ref={videoFileRef}
            type="file"
            accept="video/*"
          ...
          />
          <FormHelperText>{t("bindingGuideCreate.videosHelper")}</FormHelperText>
        </FormControl>
        ...
        {mutation.isError && (
          <Text color="red.500">{t("bindingGuideCreate.error")}</Text>
        )}
        ...
        <Button
          ...
          size="sm"
          isLoading={isLoading}
          loadingText={isUploadingVideos ? t("bindingGuideCreate.uploadingVideos") : isUploadingPhotos ? t("bindingGuideCreate.uploadingPhotos") : t("bindingGuideCreate.creating")}
        >
          {t("bindingGuideCreate.createBtn")}
        </Button>
      </VStack>
    </Box>
    </Box>
