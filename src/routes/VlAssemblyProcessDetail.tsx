...
import {
  getVlAssemblyProcessDetail, patchVlAssemblyProcess, deleteVlAssemblyProcess, syncVlAssemblyProcessFromSource,
  IVlAssemblyProcessDetail,
} from "../api";
...
export default function VlAssemblyProcessDetail() {
  ...
  const dailyOutputUrl = useMemo(
    () =>
      typeof window !== "undefined"
        ? `${window.location.origin}/vl-assembly-production/daily-outputs?vl_assembly_process=${pk}`
        : "",
    [pk]
  );

  const { data, isLoading } = useQuery<IVlAssemblyProcessDetail>({
    queryKey: ["epProcessDetail", pk],
    queryFn: () => getVlAssemblyProcessDetail(pk),
    enabled: !!pk,
  });
  ...
  const saveStatus = async (val: string) => {
    ...
    try { await patchVlAssemblyProcess(pk, { status: val }); invalidate(); }
    catch { toast({ title: t("vlAssembly.common.failedSave"), status: "error", duration: 2000, position: "bottom-right" }); }
    setSavingStatus(false);
  };

  const saveThroughputFromCycle = async (cycleRaw: string) => {
    const trimmed = cycleRaw.trim();
    if (trimmed === "") {
      try {
        await patchVlAssemblyProcess(pk, {
          cycle_time: null,
          target_qty_per_hour: null,
          daily_target_qty_8h: null,
        } as any);
        invalidate();
      } catch {
        toast({
    ...
    const sec = parseCycleTimeSeconds(trimmed);
    if (sec == null) {
      toast({
        title: t("vlAssembly.processDetail.invalidCycleTime"),
        status: "warning",
        duration: 2500,
        position: "bottom-right",
      });
    ...
    const tph = targetQtyPerHourFromCycleSeconds(sec);
    const daily = dailyTargetQty8hFromTargetPerHour(tph);
    try {
      await patchVlAssemblyProcess(pk, {
        cycle_time: trimmed,
        target_qty_per_hour: tph,
        daily_target_qty_8h: daily,
      } as any);
      invalidate();
    } catch {
      toast({
  ...
  const saveThroughputFromTargetHr = async (raw: string) => {
    ...
    const tph = Number.parseFloat(trimmed.replace(",", "."));
    if (!Number.isFinite(tph) || tph <= 0) {
      toast({
        title: t("vlAssembly.processDetail.invalidTargetPerHour"),
        status: "warning",
        duration: 2500,
        position: "bottom-right",
      });
    ...
    const tphRounded = Math.max(1, Math.round(tph));
    const sec = 3600 / tphRounded;
    const cycleStr = formatCycleTimeForApi(sec);
    const daily = dailyTargetQty8hFromTargetPerHour(tphRounded);
    try {
      await patchVlAssemblyProcess(pk, {
        target_qty_per_hour: tphRounded,
        cycle_time: cycleStr,
        daily_target_qty_8h: daily,
      } as any);
      invalidate();
  ...
  const saveText = async (field: string, val: string) => {
    if (field === "cycle_time") {
      await saveThroughputFromCycle(val);
      return;
    }
    try {
      await patchVlAssemblyProcess(pk, { [field]: val } as any);
      invalidate();
    } catch {
      toast({ title: t("vlAssembly.common.failedSave"), status: "error", duration: 2000, position: "bottom-right" });
    }
  ...
  const handleSync = async (resetFields?: string[]) => {
    setSyncing(true);
    try {
      const res = await syncVlAssemblyProcessFromSource(pk, resetFields);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await deleteVlAssemblyProcess(pk);
      broadcastVlAssemblyScheduleListCacheBust();
  ...
  const copyStandardWorkVideoUrl = async (url: string) => {
    try {
      ...
      toast({
        title: t("vlAssembly.processDetail.videoLinkCopied"),
        status: "success",
        duration: 2000,
        position: "bottom-right",
      });
      ...
      toast({
        title: t("vlAssembly.processDetail.videoLinkCopyFailed"),
        status: "error",
        duration: 2000,
        position: "bottom-right",
      });
  ...
  if (!data) return <Center minH="60vh"><Text color="gray.400">{t("vlAssembly.processDetail.notFound")}</Text></Center>;
  ...
  return (
    <>
      <Helmet><title>{`${data.code} — ${t("vlAssembly.processDetail.pageTitle")}`}</title></Helmet>
        ...
        <Box maxW="3xl" mx="auto">
              ...
              {!data.is_deleted && (
                <WrapItem>
                  <Button
                    ...
                    colorScheme="blue"
                    w={{ base: "100%", sm: "auto" }}
                    onClick={() => navigate(`/vl-assembly-production/processes/${pk}/work-order?popup=1`)}
                  >
                    {t("vlAssembly.processDetail.productionWorkOrder")}
                  </Button>
                </WrapItem>
              )}
          ...
          {qcScan && (
            ...
            >
              ...
              {t("vlAssembly.processDetail.qcScanBanner")}
            </Alert>
          )}
          ...
          <Box bg={cardBg} borderRadius="xl" border="1px solid" borderColor={borderColor} p={{ base: 4, md: 6 }} shadow="sm">
            <Flex
              ...
              flexWrap="wrap"
            >
              <VlAssemblyBadge kind="vlProcess" fontSize="sm" />
            ...
            <Box display="grid" gridTemplateColumns="repeat(auto-fill, minmax(160px, 1fr))" gap={5}>
              <InfoRow label={t("vlAssembly.processDetail.totalQty")} labelColor={labelColor}>
                <Text fontSize="sm">{data.total_qty != null ? data.total_qty.toLocaleString() : "—"}</Text>
              </InfoRow>
              <InfoRow label={t("vlAssembly.processDetail.outputQty")} labelColor={labelColor}>
              ...
              <InfoRow label={t("vlAssembly.processDetail.balance")} labelColor={labelColor}>
                <Text fontSize="sm">
                  {data.total_qty != null ? Math.max(0, data.total_qty - (data.output_qty ?? 0)).toLocaleString() : "—"}
                </Text>
              </InfoRow>
              <InfoRow label={t("vlAssembly.processDetail.status")} labelColor={labelColor}>
              ...
              <InfoRow label={t("vlAssembly.processDetail.processStart")} labelColor={labelColor}>
                <EditableDateText value={data.process_start_date} field="process_start_date" />
              </InfoRow>
              <InfoRow label={t("vlAssembly.processDetail.processFinish")} labelColor={labelColor}>
                <EditableDateText value={data.process_finish_date} field="process_finish_date" />
              </InfoRow>
              <InfoRow label={t("vlAssembly.processDetail.leadTimeDays")} labelColor={labelColor}>
                  ...
                  <Box>
                    {editingTargetHr !== null ? (
                      <Input
                        ...
                        onBlur={() => void saveThroughputFromTargetHr(editingTargetHr)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") void saveThroughputFromTargetHr(editingTargetHr);
                      ...
                      <Text
                        fontSize="sm"
                        cursor="pointer"
                        color={data.target_qty_per_hour != null ? undefined : "gray.400"}
                        _hover={{ textDecoration: "underline" }}
                        onClick={() =>
                          setEditingTargetHr(
                            data.target_qty_per_hour != null
                              ? String(data.target_qty_per_hour)
                              : ""
                          )
                        }
                      >
                        {data.target_qty_per_hour != null
                          ? `${data.target_qty_per_hour} pcs/h`
                          : "—"}
                      </Text>
                    )}
                  </Box>
              ...
              <InfoRow label={t("vlAssembly.processDetail.dailyTarget")} labelColor={labelColor}>
                <Text fontSize="sm">
                  {data.daily_target_qty_8h != null
                    ? `${data.daily_target_qty_8h} pcs`
                    : "—"}
                </Text>
              </InfoRow>
              <InfoRow label={t("vlAssembly.processDetail.machine")} labelColor={labelColor}>
                <Text fontSize="sm">{data.machine_name ?? "—"}</Text>
              </InfoRow>
              <InfoRow label={t("vlAssembly.processDetail.parentEpModule")} labelColor={labelColor}>
                <RouterLink to={`/vl-assembly-production/modules/${data.ep_module_pk}`}>
                  <Link as="span" color="blue.500" fontSize="sm">{data.ep_module_code}</Link>
                </RouterLink>
              </InfoRow>
              <InfoRow label={t("vlAssembly.processDetail.originalProcess")} labelColor={labelColor}>
                {data.source_process_info ? (
                  <HStack spacing={2} align="center" flexWrap="wrap">
                    <VlAssemblyOriginalReferenceBadges category="process" fontSize="xs" />
              ...
              {data.standard_work_video_url && (
                <InfoRow label={t("vlAssembly.processDetail.standardWorkVideo")} labelColor={labelColor}
                  ...
                  <VStack align="stretch" spacing={3}>
                    <Link href={data.standard_work_video_url} isExternal>
                      <HStack spacing={1} color="red.400">
                        <FaVideo size={13} />
                        <Text fontSize="sm">{t("vlAssembly.processDetail.watch")}</Text>
                    ...
                    {standardWorkVideoQrDataUrl && (
                      <Box>
                        <Text fontSize="sm" fontWeight="semibold" mb={1}>
                          {t("vlAssembly.processDetail.standardWorkVideoQr")}
                        </Text>
                        <Text fontSize="xs" color="gray.600" mb={2}>
                          {t("vlAssembly.processDetail.standardWorkVideoQrHint")}
                        </Text>
                        <Flex
                          align="flex-start"
                          direction={{ base: "column", sm: "row" }}
                        ...
                        >
                          ...
                          <Box flex="1" minW={{ base: "0", sm: "160px" }} w="100%">
                            ...
                            >
                              {t("vlAssembly.processDetail.copyVideoLink")}
                            </Button>
            ...
            <Box mt={6} pt={5} borderTop="1px solid" borderColor={borderColor}>
              <Box display="grid" gridTemplateColumns="repeat(auto-fill, minmax(200px, 1fr))" gap={4}>
                <InfoRow label={t("vlAssembly.processDetail.nameKo")} labelColor={labelColor}
                ...
                <InfoRow label={t("vlAssembly.processDetail.nameEn")} labelColor={labelColor}
                ...
                <InfoRow label={t("vlAssembly.processDetail.codeName")} labelColor={labelColor}
                  fieldName="code" overrideFields={overrideFields} onReset={(f) => openSyncConfirm([f])}>
                  <EditableTextField field="code" value={data.code} />
                </InfoRow>
                <InfoRow label={t("vlAssembly.processDetail.description")} labelColor={labelColor}
                ...
                <InfoRow label={t("vlAssembly.processDetail.flow")} labelColor={labelColor}
            ...
            {!data.is_deleted && <EpInspectionEntryQr targetParam="vl_assembly_process" pk={pk} />}
          </Box>
        </Box>
      </Box>
      ...
      <AlertDialog isOpen={isDeleteOpen} leastDestructiveRef={cancelRef} onClose={onDeleteClose}>
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>{t("vlAssembly.common.deleteProcessConfirm")}</AlertDialogBody>
            <AlertDialogFooter>
      ...
      <AlertDialog isOpen={isSyncOpen} leastDestructiveRef={syncCancelRef} onClose={onSyncClose}>
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>{t("vlAssembly.common.syncFromSourceConfirmBodyProcess")}</AlertDialogBody>
            <AlertDialogFooter>
