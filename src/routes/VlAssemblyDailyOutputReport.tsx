...
import {
  getVlAssemblyDailyOutputReport,
  IVlAssemblyDailyOutputReportStyleRow,
...
function StyleRow({
  ...
  row: IVlAssemblyDailyOutputReportStyleRow;
  maxQty: number;
  rank: number;
}) {
      ...
      >
        ...
        <VStack align="flex-end" spacing={0} flexShrink={0} minW="80px">
          ...
          <Text fontSize="xs" color="gray.400">
            {pct.toFixed(0)}% · {row.record_count}{t("vlAssemblyDailyReport.unitRecord")}
          </Text>
        </VStack>
        ...
        <Box bg={subBg} px={4} py={3}>
          ...
          <TableContainer>
            <Table size="sm" variant="simple">
              <Thead>
                <Tr>
                  <Th fontSize="xs">{t("vlAssemblyDailyReport.colProcessCode")}</Th>
                  <Th fontSize="xs">{t("vlAssemblyDailyReport.colProcessName")}</Th>
                  <Th isNumeric fontSize="xs">{t("vlAssemblyDailyReport.qty")}</Th>
                </Tr>
              </Thead>
              <Tbody>
                {row.processes.map((p) => (
  ...
  return (
    ...
    >
      <Text
        ...
        mb={4}
      >
        {t("vlAssemblyDailyReport.workerTitle")}
      </Text>

      {top8.length === 0 ? (
        <Center h="160px">
          <Text fontSize="sm" color="gray.400">
            {t("vlAssemblyDailyReport.noData")}
          </Text>
        </Center>
      ) : (
        <>
...
export default function VlAssemblyDailyOutputReport() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getVlAssemblyDailyOutputReport(selectedDate)
  });
