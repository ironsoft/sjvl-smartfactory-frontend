...
import {
  ...
  createVlAssemblyModuleProductionDailyOutput,
...
export default function VlAssemblyModuleProductionDailyOutputList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        onOpen();
        navigate("/vl-assembly-production/module-daily-outputs", { replace: true });
      } catch {
        if (!cancelled) {
          toast({
  ...
  const moduleFromUrl =
    searchParams.get("vl_assembly_module") ?? searchParams.get("ep_module");
  const moduleFilterNum =
    moduleFromUrl != null && moduleFromUrl !== "" ? Number(moduleFromUrl) : undefined;
  const moduleForApi =
    Number.isFinite(moduleFilterNum) && (moduleFilterNum as number) >= 1
      ? (moduleFilterNum as number)
      : undefined;
  ...
  const {
    ...
    queryFn: () =>
      getVlAssemblyModuleProductionDailyOutputs({
        schedule: schedId,
        vl_assembly_module: moduleForApi,
  ...
  const qrModuleSummaryLine = useMemo(() => {
    if (!modalSjNoPk || !modalModulePk) return "";
    const sj = sjNos.find((x) => String(x.pk) === modalSjNoPk);
    const mod = modulesForSj.find((m) => String(m.pk) === modalModulePk);
    if (!sj || !mod) return "";
    return `${sj.sj_no || "—"} / ${mod.code} — ${mod.name || ""}`;
  }, [sjNos, modulesForSj, modalSjNoPk, modalModulePk]);

  const { data: selectedModuleDetail } = useQuery({
    queryKey: ["epModuleDetail", modalModulePk],
    queryFn: () => getVlAssemblyModuleDetail(Number(modalModulePk)),
    enabled: isOpen && !!modalModulePk
  });

  const totalQty = selectedModuleDetail?.total_qty ?? null;
  const cumulative = selectedModuleDetail?.output_qty ?? 0;
  const maxQtyForNew =
  ...
  useEffect(() => {
    if (!isOpen || !qrScanPrefill || !modalModulePk) return;
    const tmr = window.setTimeout(() => qtyInputRef.current?.focus(), 400);
    return () => window.clearTimeout(tmr);
  }, [isOpen, qrScanPrefill, modalModulePk]);
  ...
  const createMut = useMutation({
    mutationFn: createVlAssemblyModuleProductionDailyOutput,
    onSuccess: (data) => {
      toast({ title: t("vlAssembly.dailyOutput.createSuccess"), status: "success" });
      resetModalForm();
      closeDisclosure();
      void Promise.all([
        queryClient.invalidateQueries({ queryKey: ["epModuleDetail", data.vl_assembly_module] }),
  ...
  const submitCreate = () => {
    ...
    if (!modalModulePk || !Number.isFinite(qty) || qty < 1) return;
    ...
    createMut.mutate({
      vl_assembly_module: Number(modalModulePk),
      qty,
  ...
  return (
    <>
      <Helmet>
        <title>{t("vlAssembly.moduleDailyOutput.pageTitle")}</title>
      </Helmet>
      <Box bg={pageBg} minH="100vh" px={{ base: 3, md: 5 }} py={6}>
        <Box maxW="1280px" mx="auto">
          <HStack
            ...
            gap={3}
          >
            <Heading size="lg">{t("vlAssembly.moduleDailyOutput.heading")}</Heading>
            <SearchInput
              onSearch={(q) => {
                setSearchQuery(q);
                setCurrentPage(1);
          ...
          <Text fontSize="sm" color="gray.500" mb={6}>
            {t("vlAssembly.moduleDailyOutput.subtitle")}
          </Text>
                ...
                <Table size="sm">
                  <Thead>
                    <Tr>
                      ...
                      <Th isNumeric>
                        {t("vlAssembly.moduleDailyOutput.colCumulativeAtSave")}
                      </Th>
                      <Th isNumeric>
                        {t("vlAssembly.moduleDailyOutput.colCumulativeActual")}
                      </Th>
                      ...
                      rows.map((r) => (
                        <Tr key={r.pk} _hover={{ bg: rowHoverBg }}>
                          <Td whiteSpace="nowrap">
                            <Text
                              as={RouterLink}
                              to={`/vl-assembly-production/module-daily-outputs/${r.pk}`}
                              color="blue.500"
                              fontWeight="medium"
                            >
                          ...
                          <Td>{r.vl_assembly_sj_no}</Td>
                          <Td>{r.vl_assembly_module_code}</Td>
                          <Td isNumeric fontWeight="semibold">
                            {r.qty}
                          </Td>
                          <Td isNumeric fontWeight="semibold" color="teal.600">
                            {r.module_cumulative_snapshot != null
                              ? r.module_cumulative_snapshot.toLocaleString()
                              : "—"}
                          </Td>
                          <Td isNumeric fontWeight="bold" color="blue.600">
                            {r.vl_assembly_module_output_qty ?? "—"}
                          </Td>
      ...
      >
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>{t("vlAssembly.moduleDailyOutput.modalTitle")}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            {qrScanPrefill && (
              <>
            ...
            {modalModulePk && selectedModuleDetail && (
              <Box
                mb={3}
                p={2}
                borderRadius="md"
              ...
              >
                ...
                <Text fontSize="xs" color="gray.500" mt={2}>
                  {t("vlAssembly.moduleDailyOutput.modalCumulativeHint")}
                </Text>
              </Box>
            )}
            <FormControl
