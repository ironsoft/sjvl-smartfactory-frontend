...
export default function VlAssemblyProcessWorkOrderPrint() {
  ...
  const { data: proc, isLoading, isError } = useQuery<IVlAssemblyProcessDetail>({
    queryKey: ["epProcessDetail", pk],
    queryFn: () => getVlAssemblyProcessDetail(pk),
    enabled: !!pk && Number.isFinite(pk),
  });

  const { data: sched } = useQuery<IVlAssemblySchedule>({
    queryKey: ["epScheduleDetail", proc?.ep_schedule_pk],
    queryFn: () => getVlAssemblyScheduleDetail(proc!.ep_schedule_pk),
  ...
  const qcInspectionUrl = useMemo(
    () =>
      typeof window !== "undefined"
        ? `${window.location.origin}/vl-assembly-production/inspections/new?vl_assembly_process=${pk}`
        : "",
    [pk]
  );
  ...
  useEffect(() => {
    ...
    const code = `VL${String(pk).padStart(8, "0")}`;
    try {
  ...
  if (!pk || !Number.isFinite(pk)) {
    return (
      <Center minH="40vh">
        <Text color="gray.500">{t("vlAssembly.workOrder.loadError")}</Text>
      </Center>
    );
  }
  ...
  if (proc.is_deleted) {
    return (
      <Center minH="40vh">
        <Text color="red.500">{t("vlAssembly.workOrder.deleted")}</Text>
      </Center>
    );
  }
  ...
  return (
    <>
      <Helmet>
        <title>{`${t("vlAssembly.workOrder.docTitle")} ${proc.code}`}</title>
        <style type="text/css">{`
          @media print {
      ...
      <Box className="no-print" px={4} py={3} bg="gray.100">
        <HStack spacing={3} flexWrap="wrap">
          <Button leftIcon={<FaArrowLeft />} size="sm" variant="outline" onClick={() => navigate(-1)}>
            {t("vlAssembly.workOrder.back")}
          </Button>
          <Button leftIcon={<FaPrint />} size="sm" colorScheme="blue" onClick={handlePrint}>
            {t("vlAssembly.workOrder.print")}
          </Button>
          <Text fontSize="sm" color="gray.600">
            {t("vlAssembly.workOrder.printHint")}
          </Text>
        </HStack>
      </Box>
        ...
        >
          ...
          <Box display="flex" flexDirection="row" alignItems="stretch" flex="1" minH="0">
            ...
            >
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblEpProcessPk")}</span>
                <span className="wo-v">{proc.pk}</span>
              </div>
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblSchedule")}</span>
                <span className="wo-v">VL Assembly #{sched?.pk ?? proc.ep_schedule_pk}</span>
              </div>
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblPo")}</span>
                <span className="wo-v">{po}</span>
              </div>
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblSjNo")}</span>
                <span className="wo-v">{sjNoLabel}</span>
              </div>
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblModule")}</span>
                <span className="wo-v">{proc.ep_module_code}</span>
              </div>
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblLine")}</span>
                <span className="wo-v">{line}</span>
              </div>
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblMachine")}</span>
                <span className="wo-v">{proc.machine_name ?? "—"}</span>
              </div>
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblQty")}</span>
                <span className="wo-v">
                  {t("vlAssembly.workOrder.qtyLine", {
              ...
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblCycle")}</span>
                <span className="wo-v">{proc.cycle_time ?? "—"}</span>
              </div>
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblTarget")}</span>
                <span className="wo-v">
              ...
              <div className="wo-row">
                <span className="wo-l">{t("vlAssembly.workOrder.lblDates")}</span>
                <span className="wo-v">
              ...
              {proc.source_process_info && (
                <div className="wo-row">
                  <span className="wo-l">{t("vlAssembly.workOrder.lblSource")}</span>
                  <span className="wo-v">
                ...
                <Box mt="1mm" pt="1mm" borderTop="0.5pt dashed #999">
                  {proc.description ? (
                    <Text fontSize="7px" fontWeight="700" mb="0.5mm">
                      {t("vlAssembly.processDetail.description")}
                    </Text>
                  ) : null}
                  {proc.description ? (
                  ...
                  {proc.flow ? (
                    <Text fontSize="7px" fontWeight="700" mb="0.5mm">
                      {t("vlAssembly.processDetail.flow")}
                    </Text>
                  ) : null}
                  {proc.flow ? (
              ...
              >
                ...
                >
                  ...
                  <Box pl="1.5mm" minW={0}>
                    <Text fontWeight="900" fontSize="8px" color="#1e3a8a" lineHeight={1.15}>
                      {t("vlAssembly.workOrder.qtyBoxTitle")}
                    </Text>
                    <Text fontSize="6px" color="#1e40af" mt="0.3mm">
                      {t("vlAssembly.workOrder.qtyBoxSub")}
                    </Text>
                  </Box>
                </HStack>
                <Box p="1mm" textAlign="center">
                  ...
                  <Text fontSize="5px" color="#1e40af" mt="0.5mm" fontWeight="600">
                    {t("vlAssembly.workOrder.qtyQrFoot")}
                  </Text>
                </Box>
              </Box>
                ...
                >
                  ...
                  <Box pl="1.5mm" minW={0}>
                    <Text fontWeight="900" fontSize="8px" color="#7f1d1d" lineHeight={1.15}>
                      {t("vlAssembly.workOrder.videoBoxTitle")}
                    </Text>
                    <Text fontSize="6px" color="#991b1b" mt="0.3mm">
                      {t("vlAssembly.workOrder.videoBoxSub")}
                    </Text>
                  </Box>
                </HStack>
                <Box p="1mm" textAlign="center">
                  {videoUrl ? (
                    ...
                    <Box py="2mm">
                      <Text fontSize="6px" color="#991b1b" fontWeight="600">
                        {t("vlAssembly.workOrder.noVideo")}
                      </Text>
                    </Box>
                  )}
                  <Text fontSize="5px" color="#991b1b" mt="0.5mm" fontWeight="600">
                    {t("vlAssembly.workOrder.videoQrFoot")}
                  </Text>
                </Box>
              </Box>
                ...
                >
                  ...
                  <Box pl="1.5mm" minW={0}>
                    <Text fontWeight="900" fontSize="8px" color="#4c1d95" lineHeight={1.15}>
                      {t("vlAssembly.workOrder.qcBoxTitle")}
                    </Text>
                    <Text fontSize="6px" color="#5b21b6" mt="0.3mm">
                      {t("vlAssembly.workOrder.qcBoxSub")}
                    </Text>
                  </Box>
                </HStack>
                <Box p="1mm" textAlign="center">
                  ...
                  <Text fontSize="5px" color="#5b21b6" mt="0.5mm" fontWeight="600">
                    {t("vlAssembly.workOrder.qcQrFoot")}
                  </Text>
                </Box>
              </Box>
            </Box>
          ...
          <Box borderTop="1pt solid #000" mt="auto" p="2mm" minH="16mm" flexShrink={0}>
            <Text fontSize="7px" color="gray.700" mb="2mm">
              {t("vlAssembly.workOrder.printedAt")}: {printedAt}
            </Text>
            <Box display="grid" gridTemplateColumns="1fr 1fr" gap="2mm">
              <Box borderBottom="0.5pt solid #000" minH="8mm">
                <Text fontSize="6px" color="gray.600">
                  {t("vlAssembly.workOrder.remark")}
                </Text>
              </Box>
              <Box borderBottom="0.5pt solid #000" minH="8mm" textAlign="right">
                <Text fontSize="6px" color="gray.600">
                  {t("vlAssembly.workOrder.signature")}
                </Text>
              </Box>
            </Box>
          </Box>
