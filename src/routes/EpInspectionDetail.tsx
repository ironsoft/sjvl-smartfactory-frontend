...
export default function EpInspectionDetail() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["epInspection", pk],
    queryFn: () => getEpInspection(pk),
  ...
  const patchMut = useMutation({
    mutationFn: (payload: Partial<IEpInspectionWritePayload>) =>
      patchEpInspection(pk, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["epInspection", pk] });
  ...
  const delMut = useMutation({
    mutationFn: () => deleteEpInspection(pk),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["epInspections"] });
      queryClient.invalidateQueries({ queryKey: ["epSchedules"] });
      toast({ title: t("epInspection.deleted"), status: "success" });
      navigate("/ep-production/inspections");
    },
    onError: () =>
