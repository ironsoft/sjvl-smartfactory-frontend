...
import {
  getVlAssemblyModuleDailyOutputReport,
  IVlAssemblyModuleDailyOutputReportStyleRow,
  IVlAssemblyDailyOutputReportWorkerRow
} from "../api";
...
function StyleRow({
  ...
  row: IVlAssemblyModuleDailyOutputReportStyleRow;
  maxQty: number;
  rank: number;
}) {
      ...
      >
        ...
        <VStack align="flex-end" spacing={0} flexShrink={0} minW="80px">
          ...
          <Text fontSize="xs" color="gray.400">
            {pct.toFixed(0)}% · {row.record_count}{t("vlAssemblyModuleDailyReport.unitRecord")}
          </Text>
        </VStack>
        ...
        <Box bg={subBg} px={4} py={3}>
          ...
          <TableContainer>
            <Table size="sm" variant="simple">
              <Thead>
                <Tr>
                  <Th fontSize="xs">{t("vlAssemblyModuleDailyReport.colModuleCode")}</Th>
                  <Th fontSize="xs">{t("vlAssemblyModuleDailyReport.colModuleName")}</Th>
                  <Th isNumeric fontSize="xs">{t("vlAssemblyModuleDailyReport.qty")}</Th>
                </Tr>
              </Thead>
              <Tbody>
                {row.modules.map((m) => (
                  <Tr key={m.module_code}>
                    <Td fontSize="xs">{m.module_code}</Td>
                    <Td fontSize="xs">{m.module_name}</Td>
                    <Td isNumeric fontSize="xs" fontWeight="bold">
                      {m.qty_today.toLocaleString()}
                    </Td>
                  </Tr>
                ))}
              </Tbody>
...
function WorkerDonutChart({
  ...
  data: IVlAssemblyDailyOutputReportWorkerRow[];
  totalQty: number;
}) {
  const cardBg = useColorModeValue("white", "gray.800");
  const border = useColorModeValue("gray.100", "gray.700");
  ...
  return (
    ...
    >
      <Text
        ...
        mb={4}
      >
        {t("vlAssemblyModuleDailyReport.workerTitle")}
      </Text>

      {top8.length === 0 ? (
        <Center h="160px">
          <Text fontSize="sm" color="gray.400">
            {t("vlAssemblyModuleDailyReport.noData")}
          </Text>
        </Center>
      ) : (
        <>
...
export default function VlAssemblyModuleDailyOutputReport() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["vl-module-daily-output-report", selectedDate],
    queryFn: () => getVlAssemblyModuleDailyOutputReport(selectedDate)
  });
  ...
  return (
    <Box minH="100vh" bg={pageBg} px={{ base: 4, md: 8 }} py={6}>
      <Helmet>
        <title>{`${t("vlAssemblyDailyReport.title")} | Sungjin MES`}</title>
      </Helmet>
      ...
      <Flex justify="space-between" align="center" mb={6} flexWrap="wrap" gap={4}>
        <Box>
          <Heading size="lg" letterSpacing="tight">
            {t("vlAssemblyDailyReport.title")}
          </Heading>
          <Text fontSize="sm" color="gray.500" mt={1}>
            {t("vlAssemblyDailyReport.subtitle")}
          </Text>
        </Box>
      ...
      {isLoading ? (
        ...
        <Center minH="40vh">
          <Text color="gray.400">{t("vlAssemblyDailyReport.noData")}</Text>
        </Center>
      ) : (
        <>
          {/* ── KPI Cards ─────────────────────────────────────────── */}
          ...
          >
            <KpiCard
              icon={<FaBoxes />}
              label={t("vlAssemblyDailyReport.kpi.totalQty")}
              value={data.kpi.total_qty_today.toLocaleString()}
              animatedValue={animTotalQty}
              subLabel={t("vlAssemblyDailyReport.kpi.totalQtySub")}
              accent="teal.400"
            />
            <KpiCard
              icon={<FaListAlt />}
              label={t("vlAssemblyDailyReport.kpi.recordCount")}
              value={data.kpi.record_count}
              animatedValue={animRecordCount}
              subLabel={t("vlAssemblyDailyReport.kpi.recordCountSub")}
              accent="blue.400"
            />
            <KpiCard
              icon={<FaTags />}
              label={t("vlAssemblyDailyReport.kpi.activeStyles")}
              value={data.kpi.active_styles_count}
              animatedValue={animStyles}
              subLabel={t("vlAssemblyDailyReport.kpi.activeStylesSub")}
              accent="purple.400"
            />
            <KpiCard
              icon={<FaCalendarDay />}
              label={t("vlAssemblyDailyReport.kpi.activeSchedules")}
              value={data.kpi.active_schedules_count}
              animatedValue={animSchedules}
              subLabel={t("vlAssemblyDailyReport.kpi.activeSchedulesSub")}
              accent="orange.400"
            />
            <KpiCard
              icon={<FaUser />}
              label={t("vlAssemblyDailyReport.kpi.activeWorkers")}
            ...
            <KpiCard
              icon={<FaClock />}
              label={t("vlAssemblyDailyReport.kpi.peakHour")}
              value={
                data.kpi.peak_hour != null
                  ? `${String(data.kpi.peak_hour).padStart(2, "0")}:00`
                  : "-"
              }
              subLabel={t("vlAssemblyDailyReport.kpi.peakHourSub")}
              accent="red.400"
            />
          </Grid>
          ...
          >
            ...
            <Box>
              <Text
                ...
                mb={4}
              >
                {t("vlAssemblyDailyReport.byStyleTitle")}
              </Text>
            ...
            >
              ...
              >
                <Text
                  ...
                  mb={5}
                >
                  {t("vlAssemblyDailyReport.hourlyTitle")}
                </Text>
                      ...
                      <HStack mt={2} spacing={4} justify="center">
                        <HStack spacing={1}>
                          ...
                          <Text fontSize="xs" color="gray.500">
                            {t("vlAssemblyDailyReport.peak")}
                          </Text>
                        </HStack>
                        <HStack spacing={1}>
                          <Box w={3} h={3} borderRadius="sm" bg="blue.400" />
                          <Text fontSize="xs" color="gray.500">
                            {t("vlAssemblyDailyReport.other")}
                          </Text>
                        </HStack>
                      </HStack>
                    )}
