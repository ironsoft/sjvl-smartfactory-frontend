...
import {
  getEpProductionDailyOutputs,
  createEpProductionDailyOutput,
  getEpSchedules,
  getEpScheduleDetail,
  getEpProcessDetail,
  IEpSchedule,
...
export default function EpProductionDailyOutputList() {
  ...
  useEffect(() => {
    const ep = searchParams.get("ep_process");
    if (!ep) return;
    const pid = Number(ep);
    ...
    (async () => {
      try {
        const proc = await getEpProcessDetail(pid);
        if (cancelled) return;
        const sched = await getEpScheduleDetail(proc.ep_schedule_pk);
        if (cancelled) return;
        ...
        onOpen();
        navigate("/ep-production/daily-outputs", { replace: true });
      } catch {
        if (!cancelled) {
          toast({
            title: t("ep.dailyOutput.qrOpenError"),
            status: "error",
            duration: 5000
          });
          navigate("/ep-production/daily-outputs", { replace: true });
  ...
  const epProcessFromUrl = searchParams.get("ep_process");
  ...
  const { data: schedules } = useQuery({
    queryKey: ["epSchedules", "daily-output-filter"],
  ...
  const { data: schedulesForOrder, isLoading: schedulesForOrderLoading } =
    useQuery({
      queryKey: ["epSchedules", "sjOrder", selectedOrder?.pk],
      queryFn: () => getEpSchedules({ sj_order: selectedOrder!.pk }),
  ...
  const { data: modalScheduleDetail, isLoading: modalSchedLoading } = useQuery({
    ...
    queryFn: () => getEpScheduleDetail(Number(modalScheduleId)),
  ...
  const { data: selectedProcessDetail } = useQuery({
    ...
    queryFn: () => getEpProcessDetail(Number(modalProcessPk)),
  ...
  const createMut = useMutation({
    mutationFn: createEpProductionDailyOutput,
    onSuccess: (data) => {
      toast({ title: t("ep.dailyOutput.createSuccess"), status: "success" });
      resetModalForm();
      closeDisclosure();
      void Promise.all([
    ...
    onError: (err: unknown) => {
      ...
      const msg =
        ax?.response?.data?.qty?.[0] ?? t("ep.dailyOutput.createError");
  ...
  return (
    <>
      <Helmet>
        <title>{t("ep.dailyOutput.pageTitle")}</title>
      </Helmet>
      <Box bg={pageBg} minH="100vh" px={{ base: 3, md: 5 }} py={6}>
        <Box maxW="1280px" mx="auto">
          <HStack
            ...
            gap={3}
          >
            <Heading size="lg">{t("ep.dailyOutput.heading")}</Heading>
            <SearchInput
              onSearch={(q) => {
                setSearchQuery(q);
                setCurrentPage(1);
          ...
          <HStack flexWrap="wrap" spacing={3} mb={4} align="flex-end">
            ...
            {!isWorker && (
              <Button
                ...
                onClick={openModal}
              >
                {t("ep.dailyOutput.addRecord")}
              </Button>
            )}
          </HStack>

          <Text fontSize="sm" color="gray.600" mb={3}>
            {t("ep.dailyOutput.totalCount", { count: totalResults })}
          </Text>
            ...
            {isLoading ? (
              ...
              <TableContainer>
                <Table size="sm">
                  <Thead>
                    <Tr>
                      <Th>{t("ep.dailyOutput.colRecordedAt")}</Th>
                      <Th>{t("ep.dailyOutput.colPo")}</Th>
                      <Th>{t("ep.dailyOutput.colSjNo")}</Th>
                      <Th>{t("ep.dailyOutput.colModule")}</Th>
                      <Th>{t("ep.dailyOutput.colProcess")}</Th>
                      <Th isNumeric>{t("ep.dailyOutput.colQty")}</Th>
                      <Th isNumeric>
                        {t("ep.dailyOutput.colCumulativeAtSave")}
                      </Th>
                      <Th isNumeric>
                        {t("ep.dailyOutput.colCumulativeActual")}
                      </Th>
                      <Th>{t("ep.dailyOutput.colBy")}</Th>
                    </Tr>
                  </Thead>
                  <Tbody>
                    {rows.length === 0 ? (
                      <Tr>
                        <Td colSpan={9}>
                          <Text color="gray.500" py={6} textAlign="center">
                            {searchQuery.trim()
                              ? t("ep.dailyOutput.noSearchResults")
                              : t("ep.dailyOutput.noRows")}
                          </Text>
                        </Td>
                      </Tr>
                    ) : (
                      rows.map((r) => (
                        <Tr key={r.pk} _hover={{ bg: rowHoverBg }}>
                          <Td whiteSpace="nowrap">
                            <Text
                              as={RouterLink}
                              to={`/ep-production/daily-outputs/${r.pk}`}
                              color="blue.500"
                              fontWeight="medium"
                            >
      ...
      >
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>{t("ep.dailyOutput.modalTitle")}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            {qrScanPrefill && (
              <>
            ...
            {!qrScanPrefill && (
              <>
                {/* SJ Order 검색 — New EP Schedule과 동일 패턴 */}
                <FormControl mb={3} isRequired>
                  <FormLabel fontSize="sm">
                    {t("ep.dailyOutput.sjOrderSearchLabel")}
                  </FormLabel>
                ...
                {schedulePickNeeded && (
                  <FormControl mb={3} isRequired>
                    <FormLabel fontSize="sm">
                      {t("ep.dailyOutput.pickEpSchedule")}
                    </FormLabel>
                    <Select
                    ...
                    <FormHelperText>
                      {t("ep.dailyOutput.pickEpScheduleHint")}
                    </FormHelperText>
                  </FormControl>
                )}
                ...
                {hasScheduleForOrder &&
                  ...
                  modalScheduleId && (
                    <Text fontSize="xs" color="gray.500" mb={3}>
                      {t("ep.dailyOutput.epScheduleLinked")}:{" "}
                ...
                >
                  <FormLabel fontSize="sm">
                    {t("ep.dailyOutput.selectProcess")}
                  </FormLabel>
                  <Select
            ...
            {modalProcessPk && selectedProcessDetail && (
              ...
              >
                <Text fontSize="xs" color="gray.600" mt={0}>
                  {t("ep.dailyOutput.modalTotalQty")}:{" "}
                ...
                <Text
                  ...
                  fontWeight="semibold"
                >
                  {t("ep.dailyOutput.processCumulativeSection")}
                </Text>
                <Text fontSize="xs" color="gray.600" mt={1} pl={2}>
                  {t("ep.dailyOutput.modalCumulativeBeforeSave")}:{" "}
                  <Text as="span" fontWeight="bold" color="teal.600">
                    {cumulative.toLocaleString()}
                  </Text>
                </Text>
                <Text fontSize="xs" color="gray.600" mt={1} pl={2}>
                  {t("ep.dailyOutput.modalCumulativeAfterSavePreview")}:{" "}
                ...
                <Text fontSize="xs" color="gray.600" mt={2}>
                  {t("ep.dailyOutput.modalRemainingQty")}:{" "}
                ...
                <Text fontSize="xs" color="gray.500" mt={2}>
                  {t("ep.dailyOutput.modalCumulativeHint")}
                </Text>
              </Box>
            )}
            <FormControl
            ...
            >
              <FormLabel fontSize="sm">{t("ep.dailyOutput.qty")}</FormLabel>
              <Input
                ref={qtyInputRef}
                type="number"
              ...
              <FormHelperText>
                {totalQty == null
                  ? t("ep.dailyOutput.noProcessTotal")
                  : maxQtyForNew === 0
                    ? t("ep.dailyOutput.qtyRemainingZero")
                    : t("ep.dailyOutput.qtyCapHint", { max: maxQtyForNew })}
              </FormHelperText>
            </FormControl>
            <FormControl mb={3}>
              <FormLabel fontSize="sm">
                {t("ep.dailyOutput.recordedAt")}
              </FormLabel>
              <Input
                type="datetime-local"
              ...
              {isWorker && (
                <FormHelperText>
                  {t("ep.dailyOutput.recordedAtWorkerLocked")}
                </FormHelperText>
              )}
            </FormControl>
            <FormControl>
              <FormLabel fontSize="sm">{t("ep.dailyOutput.remark")}</FormLabel>
              <Textarea
                value={modalRemark}
