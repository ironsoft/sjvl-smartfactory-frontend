...
function scheduleToSelectedOrder(sched: IEpSchedule): ISjOrderSearchResult {
...
export default function EpInspectionForm() {
  ...
  useEffect(() => {
    ...
    const mod = searchParams.get("ep_module");
    const sj = searchParams.get("ep_sj_no");
    if (!ep && !mod && !sj) return;
    ...
    (async () => {
      try {
        if (ep) {
          const proc = await getEpProcessDetail(Number(ep));
          if (cancelled) return;
          ...
          const m = await getEpModuleDetail(Number(mod));
          if (cancelled) return;
          const sjD = await getEpSjNoDetail(m.ep_sj_no_pk);
          if (cancelled) return;
          const sched = await getEpScheduleDetail(sjD.ep_schedule_pk);
          if (cancelled) return;
          setSelectedOrder(scheduleToSelectedOrder(sched));
          setOrderQuery(sched.sj_order_info?.sj_po_number ?? "");
          setModalScheduleId(String(sched.pk));
          ...
          const sjD = await getEpSjNoDetail(Number(sj));
          if (cancelled) return;
          const sched = await getEpScheduleDetail(sjD.ep_schedule_pk);
          if (cancelled) return;
          setSelectedOrder(scheduleToSelectedOrder(sched));
      ...
      <Box maxW="720px" mx="auto" bg={cardBg} borderWidth="1px" borderColor={border} borderRadius="md" p={6}>
        ...
        <Stack spacing={4}>
          ...
          <HStack justify="flex-end" pt={2}>
            <Button variant="ghost" onClick={() => navigate("/ep-production/inspections")}>
