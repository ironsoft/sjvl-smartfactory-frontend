...
export default function JigPublicDetail() {
  const { jigId } = useParams<{ jigId: string }>();
  ...
  const { data: item, isLoading, isError } = useQuery({
    queryKey: ["jigPublic", jigId],
    queryFn: () => getJigDetailPublic(jigId!),
    enabled: !!jigId,
    retry: false,
  });

  const { data: photos = [] } = useQuery<IFilePhotos[]>({
    queryKey: ["jigPhotosPublic", jigId],
    queryFn: () => getJigPhotosPublic(jigId!),
    enabled: !!jigId,
    retry: false,
  });
      ...
      <Box maxW="480px" mx="auto" bg={cardBg} borderRadius="xl" boxShadow="md" overflow="hidden">
        ...
        <Box bg="blue.600" px={5} py={4}>
          <Flex align="center" gap={3}>
            <Box>
              <Text fontSize="xs" color="blue.200" fontWeight="medium" mb={0.5}>Jig</Text>
