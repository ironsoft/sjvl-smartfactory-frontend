...
import {
  getEpProductionDailyOutput,
  patchEpProductionDailyOutput,
  deleteEpProductionDailyOutput,
  IEpProductionDailyOutput
} from "../api";
import { FaArrowLeft, FaTrash } from "react-icons/fa";
import { refreshEpProductionCaches } from "../lib/refreshEpProductionCaches";
import useUser from "../lib/useUser";

export default function EpProductionDailyOutputDetail() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getEpProductionDailyOutput(pk),
    enabled: Number.isFinite(pk)
  });

  const row = data as IEpProductionDailyOutput | undefined;
  ...
  useEffect(() => {
    if (!data) return;
    const d = data as IEpProductionDailyOutput;
    setQty(String(d.qty));
  ...
  const patchMut = useMutation({
    mutationFn: () => {
      ...
      return patchEpProductionDailyOutput(pk, {
        qty: q,
    ...
    onSuccess: (resp) => {
      const r = resp as IEpProductionDailyOutput;
      toast({ title: t("ep.dailyOutput.updated"), status: "success" });
      setEditing(false);
      void Promise.all([
    ...
    onError: (err: unknown) => {
      ...
      const msg =
        ax?.response?.data?.qty?.[0] ?? t("ep.dailyOutput.updateError");
      toast({ title: msg, status: "error" });
    }
  });

  const deleteMut = useMutation({
    mutationFn: () => deleteEpProductionDailyOutput(pk),
    onSuccess: () => {
      toast({ title: t("ep.dailyOutput.deleted"), status: "success" });
      navigate("/ep-production/daily-outputs");
  ...
  return (
    <>
      <Helmet>
        <title>{t("ep.dailyOutput.detailTitle", { id: pk })}</title>
      </Helmet>
      <Box bg={pageBg} minH="100vh" px={{ base: 3, md: 5 }} py={6}>
        <Box maxW="880px" mx="auto">
          <Button
            as={RouterLink}
            to="/ep-production/daily-outputs"
            size="sm"
            variant="ghost"
            leftIcon={<FaArrowLeft />}
            mb={4}
          >
            {t("ep.dailyOutput.backToList")}
          </Button>
          ...
          {isLoading || !row ? (
            ...
            <>
              <Heading size="md" mb={4}>
                {t("ep.dailyOutput.detailTitle", { id: row.pk })}
              </Heading>
              {isWorker && (
                <Alert status="info" borderRadius="md" mb={4} fontSize="sm">
                  <AlertIcon />
                  {t("ep.dailyOutput.workerReadOnlyDetail")}
                </Alert>
              )}
              <Box
                bg={cardBg}
              ...
              >
                <Text fontSize="xs" color={labelColor} mb={1}>
                  {t("ep.dailyOutput.colPo")}
                </Text>
                <Text mb={3}>{row.sj_po_number}</Text>
                <Text fontSize="xs" color={labelColor} mb={1}>
                  {t("ep.dailyOutput.colSjNo")} /{" "}
                  {t("ep.dailyOutput.colModule")} /{" "}
                  {t("ep.dailyOutput.colProcess")}
                </Text>
                <Text mb={3}>
                ...
                <Button
                  as={RouterLink}
                  to={`/ep-production/processes/${row.ep_process}`}
                  size="sm"
                  variant="link"
                  colorScheme="blue"
                  mb={2}
                >
                  {t("ep.dailyOutput.linkProcess")}
                </Button>
                <Box
                  fontSize="sm"
                  mb={4}
                ...
                >
                  ...
                  <Text mb={1} pl={2} fontSize="sm">
                    {t("ep.dailyOutput.cumulativeAtSaveRecord")}:{" "}
                  ...
                  <Text mb={1} pl={2} fontSize="sm">
                    {t("ep.dailyOutput.cumulativeActualNow")}:{" "}
                    <Text as="span" fontWeight="bold" color="blue.600">
                      {cumulative.toLocaleString()}
                    </Text>
                  </Text>
                  <Text mt={2} fontSize="sm">
                    {t("ep.dailyOutput.modalRemainingForThisEntry")}:{" "}
                ...
                >
                  ...
                  <FormHelperText>
                    {totalQty == null
                      ...
                      : maxQtyForThisRow === 0
                        ? t("ep.dailyOutput.qtyRemainingZeroEdit")
                        : t("ep.dailyOutput.qtyCapHintEdit", {
                ...
                {!isWorker && (
                  <HStack>
                    ...
                    <Button
                      ...
                      leftIcon={<FaTrash />}
                      onClick={() => {
                        if (window.confirm(t("ep.dailyOutput.deleteConfirm")))
                          deleteMut.mutate();
                      }}
                      isLoading={deleteMut.isPending}
                    >
                      {t("ep.dailyOutput.delete")}
                    </Button>
                  </HStack>
                )}

                <Text fontSize="xs" color={labelColor} mt={4}>
                  {t("ep.dailyOutput.colBy")}: {row.recorded_by_name ?? "—"}
                </Text>
              </Box>
            </>
          )}
