...
import {
  deleteAluminumMold,
  deleteAluminumMoldPhoto,
  editAluminumMold,
  getAluminumMoldDetail,
  getAluminumMoldPhotos,
  getAluminumMoldVideos,
  getStorageLocations,
  getSjStyles,
  getSjStylePhotos,
  getUploadURL,
...
export default function AluminumMoldDetail() {
  const { t } = useTranslation();
  const { aluminumMoldId } = useParams();
  ...
  const { data: videos = [], isLoading: videosLoading } = useQuery<IFileVideos[]>({
    queryKey: ["aluminumMoldVideos", aluminumMoldId],
    queryFn: () => getAluminumMoldVideos(aluminumMoldId!),
    enabled: !!aluminumMoldId
  });
  ...
  const {
    ...
    isLoading,
    error
  } = useQuery<IAluminumMoldDetail>({
    queryKey: ["aluminumMoldDetail", aluminumMoldId],
    queryFn: () => {
      if (typeof aluminumMoldId === "string") return getAluminumMoldDetail(aluminumMoldId);
      throw new Error("aluminumMoldId is not a string");
    }
  });
  ...
  useEffect(() => {
    if (item) {
      setForm({
        ...
        handed_over_dept: item.handed_over_dept ?? "",
        returned_at: item.returned_at ?? "",
        returned_by: item.returned_by ?? "",
        returned_dept: item.returned_dept ?? "",
        memo: item.memo ?? "",
      });
  ...
  const editMutation = useMutation({
    mutationFn: editAluminumMold,
    onSuccess: () => {
      toast({
        title: t("aluminumMoldDetail.updated"),
        status: "success",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
      ...
      if (aluminumMoldId)
        queryClient.invalidateQueries({ queryKey: ["aluminumMoldDetail", aluminumMoldId] });
      setIsEditing(false);
    },
    onError: () => {
      toast({
        title: t("aluminumMoldDetail.updateFailed"),
        status: "error",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteAluminumMold,
    onSuccess: () => {
      toast({
        title: t("aluminumMoldDetail.deleted"),
        status: "success",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
      ...
      navigate("/aluminum-molds");
    },
    onError: () => {
      toast({
        title: t("aluminumMoldDetail.deleteFailed"),
        status: "error",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
  ...
  const deletePhotoMutation = useMutation({
    mutationFn: ({ photoPk }: { photoPk: string }) =>
      deleteAluminumMoldPhoto({ aluminumMoldId: aluminumMoldId!, photoPk }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["aluminumMoldPhotos", aluminumMoldId] });
      queryClient.invalidateQueries({ queryKey: ["aluminumMoldPhotos"] });
      toast({
        title: t("aluminumMoldDetail.photoDeleted"),
        status: "success",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
    ...
    onError: () => {
      toast({
        title: t("aluminumMoldDetail.photoDeleteFailed"),
        status: "error",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
  ...
  const deleteVideoMutation = useMutation({
    mutationFn: ({ videoPk }: { videoPk: string }) =>
      deleteAluminumMoldVideo({ aluminumMoldId: aluminumMoldId!, videoPk }),
    onSuccess: () => {
  ...
  const createAluminumMoldVideoMutation = useMutation({
    mutationFn: createAluminumMoldVideo,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["aluminumMoldVideos", aluminumMoldId] });
  ...
  const getVideoDataMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      createAluminumMoldVideoMutation.mutate({ VideoFile: videoURL, ThumbnailFile: thumbnailURL, aluminumMoldId: aluminumMoldId! });
    }
  });
  ...
  const handlePrintLabel = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>${item?.name ?? "Aluminum Mold Label"}</title>
          <style>
  ...
  const handleGenerateQR = async () => {
    if (!aluminumMoldId) return;
    setIsGeneratingQR(true);
    try {
      const existingQRPhotos = photos.filter((p) => p.description === "QR Code");
      for (const qrPhoto of existingQRPhotos) {
        await deleteAluminumMoldPhoto({ aluminumMoldId, photoPk: qrPhoto.pk });
      }
      // 공개 페이지 URL로 QR 코드 생성 (로그인 없이 스캔 가능)
      const publicUrl = `${window.location.origin}/public/aluminum-molds/${aluminumMoldId}`;
      const dataUrl: string = await QRCode.toDataURL(publicUrl, { width: 400, margin: 2 });
      const res = await fetch(dataUrl);
      const blob = await res.blob();
      const qrFile = new File([blob], `aluminum-mold-qr-${aluminumMoldId}.png`, { type: "image/png" });
      ...
      await createAluminumMoldPhoto({ file: cfUrl, aluminumMoldId, description: "QR Code" });
      queryClient.invalidateQueries({ queryKey: ["aluminumMoldPhotos", aluminumMoldId] });
      queryClient.invalidateQueries({ queryKey: ["aluminumMoldPhotos"] });
  ...
  const uploadOnePhoto = async (file: File) => {
    ...
    const cfResult: any = await uploadImage({ file: dt.files, uploadURL: urlData.uploadURL });
    const cfUrl = `https://imagedelivery.net/mzmXhxWLR9jzdX8u9g4BBQ/${cfResult.result.id}/public`;
    await createAluminumMoldPhoto({ file: cfUrl, aluminumMoldId: aluminumMoldId!, description: item?.name ?? "" });
  };
  ...
  const handleUploadNewPhotos = async () => {
    ...
    try {
      ...
      setNewFiles([]);
      setNewPreviews([]);
      toast({ title: t("aluminumMoldDetail.photosUploaded"), status: "success", duration: 2000, isClosable: true, position: "bottom-right" });
    } catch {
      toast({ title: t("aluminumMoldDetail.photosUploadFailed"), status: "warning", duration: 3000, isClosable: true, position: "bottom-right" });
    } finally {
  ...
  if (isLoading) {
    return (
      <>
        <Helmet><title>Loading... Aluminum Mold Detail</title></Helmet>
  ...
  if (error || !item) {
    return (
      <>
        <Helmet><title>Aluminum Mold Not Found</title></Helmet>
        <Center minH="60vh">
          <Box textAlign="center">
            <Heading size="md" mb={2}>{t("aluminumMoldDetail.notFound")}</Heading>
            <Text color="gray.500" mb={4}>{t("aluminumMoldDetail.notFoundDesc")}</Text>
            <HStack display={"flex"} alignItems={"center"} gap={2}>
              <Button leftIcon={<FaArrowLeft />} variant="ghost" size="sm" onClick={() => navigate("/aluminum-molds")}>
                {t("aluminumMoldDetail.backToListBtn")}
              </Button>
            </HStack>
          </Box>
        </Center>
  ...
  return (
    <>
      <Helmet>
        <title>{item.name ?? "Aluminum Mold Detail"}</title>
      </Helmet>
      <Box bg={pageBg} flex="1" minH="calc(100vh - 120px)" py={{ base: 6, md: 10 }}>
        <Box maxW={{ base: "3xl", lg: "5xl" }} mx="auto" px={{ base: 4, md: 8 }}>
          <HStack justify="space-between" align="flex-start" mb={4}>
            <Button as={RouterLink} to="/aluminum-molds" variant="ghost" size="sm" leftIcon={<FaArrowLeft />}>
              {t("aluminumMoldDetail.backToList")}
            </Button>
            <Button
              size="sm"
              colorScheme="teal"
          ...
          <AlertDialog isOpen={isDeleteOpen} leastDestructiveRef={cancelRef} onClose={() => setIsDeleteOpen(false)}>
            <AlertDialogOverlay>
              <AlertDialogContent>
                <AlertDialogHeader fontSize="lg" fontWeight="bold">{t("aluminumMoldDetail.deleteTitle")}</AlertDialogHeader>
                <AlertDialogBody>{t("aluminumMoldDetail.deleteConfirm")}</AlertDialogBody>
                <AlertDialogFooter>
                  <Button ref={cancelRef} onClick={() => setIsDeleteOpen(false)}>{t("aluminumMoldDetail.cancel")}</Button>
                  <Button colorScheme="red" ml={3} isLoading={deleteMutation.status === "pending"} onClick={() => { deleteMutation.mutate(aluminumMoldId!); setIsDeleteOpen(false); }}>
                    {t("aluminumMoldDetail.delete")}
                  </Button>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialogOverlay>
          ...
          <AlertDialog isOpen={isQRConfirmOpen} leastDestructiveRef={qrCancelRef} onClose={() => setIsQRConfirmOpen(false)} isCentered>
            <AlertDialogOverlay>
              <AlertDialogContent>
                <AlertDialogHeader fontSize="lg" fontWeight="bold">{t("aluminumMoldDetail.qrExistsTitle")}</AlertDialogHeader>
                <AlertDialogBody>{t("aluminumMoldDetail.qrExistsConfirm")}</AlertDialogBody>
                <AlertDialogFooter>
                  <Button ref={qrCancelRef} onClick={() => setIsQRConfirmOpen(false)}>{t("aluminumMoldDetail.cancel")}</Button>
                  <Button colorScheme="teal" ml={3} onClick={() => { setIsQRConfirmOpen(false); handleGenerateQR(); }}>
                    {t("aluminumMoldDetail.qrExistsConfirmBtn")}
                  </Button>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialogOverlay>
          ...
          <Box bg={cardBg} borderRadius="xl" boxShadow="lg" p={{ base: 5, md: 8 }}>
            <Box mb={4} position="relative">
              ...
              <Heading size="lg" mb={2}>{item.name ?? "Aluminum Mold Detail"}</Heading>
                ...
                <Box>
                    ...
                    {photos.filter((p) => p.description !== "QR Code").map((photo) => (
                      <Box key={photo.pk} position="relative">
                        <Box rounded="md" overflow="hidden" border="1px solid" borderColor="gray.200" w="120px" h="120px" cursor={isEditing ? "default" : "pointer"} onClick={() => !isEditing && setSelectedPhoto(photo)} _hover={!isEditing ? { opacity: 0.8, transform: "scale(1.03)", transition: "all 0.2s" } : {}}>
                          <Image src={photo.file} alt={photo.description ?? "aluminum mold photo"} w="100%" h="100%" objectFit="cover" />
                        </Box>
                        {isEditing && (
                  ...
                  {photos.filter((p) => p.description !== "QR Code").length === 0 && newPreviews.length === 0 && !isEditing && (
                    <Box w="100px" h="100px" rounded="md" border="1px dashed" borderColor="gray.300" display="flex" alignItems="center" justifyContent="center">
                      <Text fontSize="xs" color="gray.400" textAlign="center">{t("aluminumMoldDetail.noPhotos")}</Text>
                    </Box>
                  )}
                    ...
                    <HStack mt={2} spacing={2}>
                      ...
                      {newFiles.length > 0 && (
                        <Button size="xs" colorScheme="blue" isLoading={isUploadingPhoto} loadingText="Uploading..." onClick={handleUploadNewPhotos}>
                          {t("aluminumMoldDetail.uploadPhotos")} ({newFiles.length})
                        </Button>
                      )}
                    </HStack>
                  )}
                ...
                <Box>
                    ...
                    {videos.map((video) => (
                      <Box key={video.pk} position="relative">
                        <Box rounded="md" overflow="hidden" border="1px solid" borderColor="gray.200" w="120px" h="80px" cursor="pointer" position="relative" onClick={() => !isEditing && setSelectedVideo(video.VideoFile)} _hover={!isEditing ? { opacity: 0.8, transform: "scale(1.03)", transition: "all 0.2s" } : {}}>
                          {video.ThumbnailFile ? (
                            <Image src={video.ThumbnailFile} alt={video.description ?? "aluminum mold video"} w="100%" h="100%" objectFit="cover" />
                          ) : (
                  ...
                  {isEditing && (
                    <HStack mt={2} spacing={2}>
                      ...
                      {pendingVideoFile && (
                        <Button size="xs" colorScheme="teal" isLoading={isUploadingVideo || uploadVideoURLMutation.isPending || uploadVideoMutation.isPending || getVideoDataMutation.isPending || createAluminumMoldVideoMutation.isPending} loadingText="Uploading..." onClick={handleUploadVideo}>
                          Upload Video
                        </Button>
                      )}
                    </HStack>
            ...
            <Stack spacing={4}>
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.name")}</Text>
                {isEditing ? (
              ...
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.serialNumber")}</Text>
                <Text>{item.serial_number ?? "-"}</Text>
              </Box>

              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.status")}</Text>
                {isEditing ? (
                  <Select size="sm" value={form.status} onChange={(e) => setForm((p) => ({ ...p, status: e.target.value }))}>
                    <option value="">{t("aluminumMoldDetail.selectStatus")}</option>
              ...
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.description")}</Text>
                {isEditing ? (
              ...
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.location")}</Text>
                {isEditing ? (
                  <Select size="sm" value={form.location} onChange={(e) => setForm((p) => ({ ...p, location: e.target.value }))}>
                    <option value="">{t("aluminumMoldDetail.noLocation")}</option>
                    {locations.map((loc) => (
                  ...
                  <HStack spacing={2} align="center">
                    <Text>
                      {locationDetail.code ?? "-"} ({t("aluminumMoldDetail.locationZone")}{" "}
                      {locationDetail.zone ?? "-"} · {t("aluminumMoldDetail.locationShelf")} {locationDetail.shelf ?? "-"}{" "}
                      · {t("aluminumMoldDetail.locationSlot")} {locationDetail.slot ?? "-"})
                    </Text>
                    <Button size="xs" leftIcon={<RiFlashlightFill />} colorScheme="yellow" variant="outline" isDisabled={lightTimer > 0} isLoading={lightOnMutation.isPending} onClick={() => aluminumMoldId && lightOnMutation.mutate(aluminumMoldId)}>
                      {lightTimer > 0 ? `${lightTimer}s` : t("aluminumMoldList.lightOn")}
                    </Button>
                  </HStack>
                ) : (
              ...
              {/* 제작일 */}
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.manufacturedDate")}</Text>
                {isEditing ? (
              ...
              {/* 인계 정보 */}
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.handedOverAt")}</Text>
                {isEditing ? (
                  <Input size="sm" type="date" value={form.handed_over_at} onChange={(e) => setForm((p) => ({ ...p, handed_over_at: e.target.value }))} />
                ) : (
                  <Text>{item.handed_over_at ?? "-"}</Text>
                )}
              </Box>
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.handedOverBy")}</Text>
                {isEditing ? (
                  <Input size="sm" value={form.handed_over_by} onChange={(e) => setForm((p) => ({ ...p, handed_over_by: e.target.value }))} />
                ) : (
                  <Text>{item.handed_over_by || "-"}</Text>
                )}
              </Box>
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.handedOverDept")}</Text>
                {isEditing ? (
                  <Input size="sm" value={form.handed_over_dept} onChange={(e) => setForm((p) => ({ ...p, handed_over_dept: e.target.value }))} />
                ) : (
                  <Text>{item.handed_over_dept || "-"}</Text>
                )}
              </Box>

              {/* 반납 정보 */}
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.returnedAt")}</Text>
                {isEditing ? (
                  <Input size="sm" type="date" value={form.returned_at} onChange={(e) => setForm((p) => ({ ...p, returned_at: e.target.value }))} />
                ) : (
                  <Text>{item.returned_at ?? "-"}</Text>
                )}
              </Box>
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.returnedBy")}</Text>
                {isEditing ? (
                  <Input size="sm" value={form.returned_by} onChange={(e) => setForm((p) => ({ ...p, returned_by: e.target.value }))} />
                ) : (
                  <Text>{item.returned_by || "-"}</Text>
                )}
              </Box>
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.returnedDept")}</Text>
                {isEditing ? (
                  <Input size="sm" value={form.returned_dept} onChange={(e) => setForm((p) => ({ ...p, returned_dept: e.target.value }))} />
                ) : (
                  <Text>{item.returned_dept || "-"}</Text>
                )}
              </Box>

              {/* 메모 */}
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.memo")}</Text>
                {isEditing ? (
                  <Textarea size="sm" value={form.memo} onChange={(e) => setForm((p) => ({ ...p, memo: e.target.value }))} rows={3} />
                ) : (
                  <Text whiteSpace="pre-wrap">{item.memo || "-"}</Text>
                )}
              </Box>

              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.createdAt")}</Text>
                <Text>{formatDate(item.created_at)}</Text>
              </Box>
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>{t("aluminumMoldDetail.updatedAt")}</Text>
                <Text>{formatDate(item.updated_at)}</Text>
              </Box>

              {isEditing && (
                <HStack justify="flex-end" pt={2}>
                  <Button size="sm" variant="ghost" onClick={() => setIsEditing(false)}>{t("aluminumMoldDetail.cancel")}</Button>
                  <Button size="sm" colorScheme="blue" onClick={onSave} isLoading={editMutation.status === "pending"}>{t("aluminumMoldDetail.save")}</Button>
                </HStack>
              )}
            </Stack>
          </Box>
        ...
        <HStack justify="center" spacing={4} mt={6}>
          <Button size="sm" variant="outline" onClick={() => setIsEditing((p) => !p)}>
            {isEditing ? t("aluminumMoldDetail.cancel") : t("aluminumMoldDetail.edit")}
          </Button>
      ...
      <Modal isOpen={!!selectedPhoto} onClose={() => setSelectedPhoto(null)} size="xl" isCentered>
        ...
        <ModalContent bg="transparent" boxShadow="none">
          ...
          <ModalBody p={0}>
            {selectedPhoto && (
              <Image src={selectedPhoto.file} alt={selectedPhoto.description ?? "aluminum mold photo"} w="100%" maxH="80vh" objectFit="contain" rounded="md" />
            )}
