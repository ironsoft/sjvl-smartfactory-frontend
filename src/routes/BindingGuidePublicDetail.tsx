...
import {
  getBindingGuideDetailPublic,
  getBindingGuidePhotosPublic,
  IFilePhotos,
} from "../api";

export default function BindingGuidePublicDetail() {
  const { bindingGuideId } = useParams<{ bindingGuideId: string }>();
  ...
  const { data: item, isLoading, isError } = useQuery({
    queryKey: ["bindingGuidePublic", bindingGuideId],
    queryFn: () => getBindingGuideDetailPublic(bindingGuideId!),
    enabled: !!bindingGuideId,
    retry: false,
  });

  const { data: photos = [] } = useQuery<IFilePhotos[]>({
    queryKey: ["bindingGuidePhotosPublic", bindingGuideId],
    queryFn: () => getBindingGuidePhotosPublic(bindingGuideId!),
    enabled: !!bindingGuideId,
    retry: false,
  });
  ...
  return (
    <Box bg={pageBg} minH="100vh" px={{ base: 4, md: 8 }} py={8}>
      <Helmet>
        <title>{item.name} — Binding Guide</title>
      </Helmet>
      ...
      <Box maxW="480px" mx="auto" bg={cardBg} borderRadius="xl" boxShadow="md" overflow="hidden">
        ...
        <Box bg="teal.600" px={5} py={4}>
          <Flex align="center" gap={3}>
            <Box>
              <Text fontSize="xs" color="teal.200" fontWeight="medium" mb={0.5}>Binding Guide</Text>
