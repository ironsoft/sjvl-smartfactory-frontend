...
export default function JigDetail() {
  const { t } = useTranslation();
  const { jigId } = useParams();
  ...
  const { data: videos = [], isLoading: videosLoading } = useQuery<IFileVideos[]>({
    queryKey: ["jigVideos", jigId],
    queryFn: () => getJigVideos(jigId!),
    enabled: !!jigId
  });
  ...
  const {
    ...
    queryKey: ["jigDetail", jigId],
    queryFn: () => {
      if (typeof jigId === "string") return getJigDetail(jigId);
      throw new Error("jigId is not a string");
    }
  });
  ...
  const editMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (jigId)
        queryClient.invalidateQueries({ queryKey: ["jigDetail", jigId] });
      setIsEditing(false);
    },
    onError: () => {
      toast({
        title: t("jigDetail.updateFailed"),
        status: "error",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteJig,
    onSuccess: () => {
      toast({
        title: t("jigDetail.deleted"),
        status: "success",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
    ...
    onError: () => {
      toast({
        title: t("jigDetail.deleteFailed"),
        status: "error",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
  ...
  const deletePhotoMutation = useMutation({
    mutationFn: ({ photoPk }: { photoPk: string }) =>
      deleteJigPhoto({ jigId: jigId!, photoPk }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["jigPhotos", jigId] });
      queryClient.invalidateQueries({ queryKey: ["jigPhotos"] });
      toast({
        title: t("jigDetail.photoDeleted"),
        status: "success",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
    ...
    onError: () => {
      toast({
        title: t("jigDetail.photoDeleteFailed"),
        status: "error",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
  ...
  const deleteVideoMutation = useMutation({
    mutationFn: ({ videoPk }: { videoPk: string }) =>
      deleteJigVideo({ jigId: jigId!, videoPk }),
    onSuccess: () => {
  ...
  // 비디오 생성 (Django 저장)
  const createJigVideoMutation = useMutation({
    mutationFn: createJigVideo,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["jigVideos", jigId] });
  ...
  const handleGenerateQR = async () => {
    ...
    try {
      ...
      for (const qrPhoto of existingQRPhotos) {
        await deleteJigPhoto({ jigId, photoPk: qrPhoto.pk });
      }

      // 2) 공개 페이지 URL로 QR 코드 생성 (로그인 없이 스캔 가능)
      const publicUrl = `${window.location.origin}/public/jigs/${jigId}`;
      ...
      const qrFile = new File([blob], `jig-qr-${jigId}.png`, {
        type: "image/png"
      });
      ...
      // 6) Django에 저장
      await createJigPhoto({ file: cfUrl, jigId, description: "QR Code" });
  ...
  const handleUploadNewPhotos = async () => {
    ...
    try {
      ...
      setNewPreviews([]);
      toast({
        title: t("jigDetail.photosUploaded"),
        status: "success",
        duration: 2000,
        isClosable: true,
        position: "bottom-right"
      ...
      toast({
        title: t("jigDetail.photosUploadFailed"),
        status: "warning",
        duration: 3000,
        isClosable: true,
        position: "bottom-right"
        ...
        <Center minH="60vh">
          <Box textAlign="center">
            <Heading size="md" mb={2}>
              {t("jigDetail.notFound")}
            </Heading>
            <Text color="gray.500" mb={4}>
              {t("jigDetail.notFoundDesc")}
            </Text>
            <HStack display={"flex"} alignItems={"center"} gap={2}>
              <Button
                ...
                variant="ghost"
                size="sm"
                onClick={() => navigate("/jigs")}
              >
                {t("jigDetail.backToListBtn")}
              </Button>
            </HStack>
          </Box>
        </Center>
    ...
    <>
      ...
      >
        ...
        >
            ...
            <AlertDialogOverlay>
              <AlertDialogContent>
                <AlertDialogHeader fontSize="lg" fontWeight="bold">
                  {t("jigDetail.deleteTitle")}
                </AlertDialogHeader>
                <AlertDialogBody>
                  {t("jigDetail.deleteConfirm")}
                </AlertDialogBody>
                <AlertDialogFooter>
                  <Button
                    ref={cancelRef}
          ...
          >
            <AlertDialogOverlay>
              <AlertDialogContent>
                <AlertDialogHeader fontSize="lg" fontWeight="bold">
                  {t("jigDetail.qrExistsTitle")}
                </AlertDialogHeader>
                <AlertDialogBody>
                  {t("jigDetail.qrExistsConfirm")}
                </AlertDialogBody>
                <AlertDialogFooter>
                  <Button
                    ref={qrCancelRef}
                  ...
                  >
                    {t("jigDetail.qrExistsConfirmBtn")}
                  </Button>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialogOverlay>
                  ...
                  {isEditing && (
                    <HStack mt={2} spacing={2}>
                      ...
                      {newFiles.length > 0 && (
                        ...
                        >
                          {t("jigDetail.uploadPhotos")} ({newFiles.length})
                        </Button>
                      )}
                    </HStack>
                  )}
                    ...
                    <HStack mt={2} spacing={2}>
                      ...
                      {pendingVideoFile && (
                        <Button
                          size="xs"
                          colorScheme="teal"
                          isLoading={isUploadingVideo || uploadVideoURLMutation.isPending || uploadVideoMutation.isPending || getVideoDataMutation.isPending || createJigVideoMutation.isPending}
                          loadingText="Uploading..."
            ...
            <Stack spacing={4}>
              ...
              <Box>
                ...
                {isEditing ? (
                  ...
                  >
                    <option value="">{t("jigDetail.selectShape")}</option>
                    {SHAPE_OPTIONS.map((o) => (
              ...
              <Box>
                ...
                {isEditing ? (
                  ...
                  >
                    <option value="">{t("jigDetail.selectMaterial")}</option>
              ...
              <Box>
                ...
                {isEditing ? (
                  ...
                  >
                    <option value="">{t("jigDetail.selectStatus")}</option>
              ...
              <Box>
                ...
                {isEditing ? (
                  ...
                  >
                    <option value="">{t("jigDetail.noLocation")}</option>
                    {locations.map((loc) => (
                  ...
                  <HStack spacing={2} align="center">
                    <Text>
                      {locationDetail.code ?? "-"} ({t("jigDetail.locationZone")}{" "}
                      {locationDetail.zone ?? "-"} ·{" "}
                      {t("jigDetail.locationShelf")} {locationDetail.shelf ?? "-"}{" "}
                      · {t("jigDetail.locationSlot")} {locationDetail.slot ?? "-"}
                      )
                    </Text>
                    <Button
                      size="xs"
                      ...
                      isLoading={jigLightOnMutation.isPending}
                      onClick={() => jigId && jigLightOnMutation.mutate(jigId)}
                    >
                      {lightTimer > 0 ? `${lightTimer}s` : t("jigList.lightOn")}
                    </Button>
                  </HStack>
                ) : (
              ...
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>
                  {t("jigDetail.createdAt")}
                </Text>
                <Text>{formatDate(jig.created_at)}</Text>
              </Box>
              <Box>
                <Text fontSize="sm" color={labelColor} mb={1}>
                  {t("jigDetail.updatedAt")}
                </Text>
        ...
        <HStack justify="center" spacing={4} mt={6}>
          ...
          >
            {isEditing ? t("jigDetail.cancel") : t("jigDetail.edit")}
          </Button>
          <Button
            size="sm"
            colorScheme="red"
